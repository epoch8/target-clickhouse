...
```bash
uv tool install target-clickhouse
```

Install from GitHub:

```bash
uv tool install git+https://github.com/ORG_NAME/target-clickhouse.git@main
```
...
tap-smoke-test | target-clickhouse --config /path/to/target-clickhouse-config.json
```
...
You can also test the `target-clickhouse` CLI interface directly using `uv run`:
...
meltano run tap-smoke-test target-clickhouse
```
